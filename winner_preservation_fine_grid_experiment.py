...
# Pre-specified fine grid focused on trades that have already reached +2R.
# The 50-DMA routine exit is disabled only after the trigger; initial stop stays active.
VARIANTS={
    'baseline': {'trail': None, 'atr_mult': None, 'hybrid': False},
    'atr2_after_2r': {'trail': 'atr', 'atr_mult': 2.0, 'hybrid': False},
    'atr2_5_after_2r': {'trail': 'atr', 'atr_mult': 2.5, 'hybrid': False},
    'atr3_after_2r': {'trail': 'atr', 'atr_mult': 3.0, 'hybrid': False},
    'atr3_5_after_2r': {'trail': 'atr', 'atr_mult': 3.5, 'hybrid': False},
    'hybrid_50dma_atr2_5_after_2r': {'trail': 'atr', 'atr_mult': 2.5, 'hybrid': True},
}
...
def run_variant(files, market, start, end, near, tighten, vm, name):
    ...
    for dt in market.index[(market.index>=start)&(market.index<=end)]:
        for sym in list(pos):
            ...
            if peak_r >= 2.0 and cfg['trail']=='atr':
                atr_stop=px-cfg['atr_mult']*float(f['atr14'].loc[dt])
                effective_stop=max(effective_stop,atr_stop)
                reason=f"atr{cfg['atr_mult']:g}_trail_winner"

            if low <= effective_stop:
                proceeds,trade=close_trade(pos[sym],effective_stop,dt,reason)
                cash+=proceeds; trades.append(trade); del pos[sym]; continue

            # Baseline keeps the original 50-DMA close. The fine-grid variants
            # suppress it after +2R. The hybrid retains it only if it is above
            # the ATR stop, providing a stricter trend exit.
            if peak_r < 2.0 or cfg['hybrid']:
                if np.isfinite(ma50) and px<ma50:
                    if not cfg['hybrid'] or effective_stop >= ma50:
...
def main():
    ...
    df=pd.DataFrame(rows); df.to_csv(out/'winner_preservation_fine_grid_results.csv',index=False)
    pd.concat(all_trades,ignore_index=True).to_csv(out/'winner_preservation_fine_grid_trades.csv',index=False)
    ...
    summary.to_csv(out/'winner_preservation_fine_grid_summary.csv',index=False)
    # Also produce per-window/config rows for OOS inspection.
    df.sort_values(['start','exit_variant','near_high','tighten','vol_mult']).to_csv(out/'winner_preservation_fine_grid_detailed.csv',index=False)
    print('WINNER PRESERVATION FINE GRID SUMMARY'); print(summary.to_string(index=False))
